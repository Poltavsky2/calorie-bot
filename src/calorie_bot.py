...
{
  "type": "food",
  "calculation_reasoning": "Здесь СТРОГО напиши свои рассуждения по расчету КБЖУ. Шаг 1: Оцени вес макронутриентов. Сумма (Белки + Жиры + Углеводы) СТРОГО <= 100г. Шаг 2: Посчитай калории: (Белки × 4) + (Углеводы × 4) + (Жиры × 9) = Калории. Убедись, что математика сошлась.",
...
КРИТИЧЕСКИ ВАЖНО ДЛЯ КБЖУ: Нейросети часто ошибаются в математике. Поэтому используй поле calculation_reasoning для рассуждений ПЕРЕД тем, как выдать nutrition. Сумма макронутриентов (Белки + Жиры + Углеводы) СТРОГО <= 100г. Если получается 115г в 100г продукта — это физически невозможно! Вода тоже имеет вес, поэтому макронутриентов обычно меньше 100г.
