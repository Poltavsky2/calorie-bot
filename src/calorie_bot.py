...
{
  "type": "food",
  "calculation_reasoning": "Здесь СТРОГО напиши рассуждения. Шаг 1: Вспомни табличное КБЖУ (USDA) сырых ингредиентов. Шаг 2: Учти коэффициенты изменения веса при готовке (вода/жир). Шаг 3: Оцени вес готовых ингредиентов и макросы для ВСЕЙ порции. Шаг 4: Раздели макросы на вес порции и умножь на 100, чтобы получить значения НА 100 ГРАММ. Шаг 5: Проверь, что (Белки+Жиры+Углеводы) <= 100г. Шаг 6: Калории = (Б×4)+(У×4)+(Ж×9).",
...
(Примечание: В объекте nutrition ВСЕ значения (калории, белки, жиры, углеводы) ДОЛЖНЫ БЫТЬ СТРОГО НА 100 ГРАММ готового продукта/блюда (в том виде, в котором оно на тарелке), а не сухих/сырых ингредиентов!
СУПЕР КРИТИЧЕСКИ ВАЖНО: Для оценки КБЖУ ОБЯЗАТЕЛЬНО вспоминай и используй стандартные табличные значения из проверенных баз данных (USDA FoodData Central, справочники нутрициологии). Твои цифры не должны расходиться с реальными научными данными! Если блюдо составное, разложи его на ингредиенты, вспомни их табличное КБЖУ, учти коэффициенты уварки/ужарки (впитывание воды крупами, макаронами или выпаривание воды из мяса) и высчитай точную пищевую ценность готового блюда.
КРИТИЧЕСКИ ВАЖНО ДЛЯ КБЖУ: Нейросети часто пишут в блок nutrition значения для ВСЕЙ порции, если их сумма меньше 100г. ЭТО ОШИБКА! Если порция весит 500г, ты должен вычислить макросы для 500г, а затем РАЗДЕЛИТЬ ИХ НА 5, чтобы получить значения строго на 100г. Поэтому используй поле calculation_reasoning для рассуждений ПЕРЕД тем, как выдать nutrition. Сумма макронутриентов на 100г продукта обычно сильно меньше 100г из-за наличия воды (например, в твороге 5% всего ~25г макронутриентов на 100г, остальное - вода).
